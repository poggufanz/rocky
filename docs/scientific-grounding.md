...
That finding motivates Rocky's longer arc, but it does not establish Rocky's effect. The v0.5 dictionary connects a user's recorded intent to the mechanism in their own work. The dictionary, bounded agent-change capture, ambiguity handling, digest, quiz, and proactive questions are implemented in the 0.5 release line; current package 0.5.2 also provides terminal failure/fix memory, `rocky watch`, read-only MCP, and optional local interpretation of recall evidence.
