...
5. **Long-running envelope** — the reader has a 32 MiB file cap, a 1 MiB line cap, and a 50,000-record reference envelope only on the exact Linux Node 22 runtime. Windows and other runtimes use an explicit 10,000-record supported cap until their own scorecard meets the 150 MiB RSS budget; larger files stay bounded and report degraded coverage. Above any cap (including a 250,000-record stress fixture) it stops at the boundary and every MCP/diagnostic answer carries `memoryCoverage` (`version`, `scanned`, `skipped`, `truncated`, `complete`); skipped corrupt or oversized lines are counted, never silently treated as complete. A single immutable snapshot is reused only after a full bounded content witness confirms every byte that can affect the bounded answer plus file identity, size, and metadata.
