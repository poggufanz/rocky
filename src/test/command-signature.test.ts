...
test("wrapper programs and env assignments are transparent, so opposite subcommands do not collapse", () => {
  ...
  assert.equal(commandSignature("sudo systemctl restart nginx"), "systemctl restart");
  assert.equal(commandSignature("sudo systemctl status nginx"), "systemctl status");
  assert.notEqual(
  ...
  assert.equal(commandSignature("time npm run build"), "npm run");
});

test("a flag's value is not the subcommand", () => {
  // `docker --context prod ps` was a STRONG fix for a failed `docker ... build .`
  assert.equal(commandSignature("docker --context prod build ."), "docker build");
  assert.equal(commandSignature("docker --context prod ps"), "docker ps");
  assert.notEqual(
    commandSignature("docker --context prod build ."),
    commandSignature("docker --context prod ps"),
  );

  assert.equal(commandSignature("npm --prefix ./app run build"), "npm run");
  assert.equal(commandSignature("npm --prefix ./app test"), "npm test");
