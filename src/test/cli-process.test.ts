...
test("release truth keeps branch and immutable-tag modes distinct", async () => {
  ...
  const releaseHead = "4aa845279983ec7c7daaf869f6e0cdf370a6dc25";
  ...
  try {
    ...
    const runner = (_root: string, args: string[], options?: { env?: NodeJS.ProcessEnv; gitExecutable?: string }): string | undefined => {
      ...
      if (command === "rev-parse --verify refs/tags/v0.7.3^{commit}") return releaseHead;
    ...
    assert.ok(calls.some((args) => args.join(" ") === "rev-parse --verify refs/tags/v0.7.3^{commit}"));
