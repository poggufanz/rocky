...
test("release truth keeps branch and immutable-tag modes distinct", async () => {
  ...
  const releaseHead = "08943dd76a4d8f58232d23ca3046bb86191c0456";
  ...
  try {
    ...
    const runner = (_root: string, args: string[], options?: { env?: NodeJS.ProcessEnv; gitExecutable?: string }): string | undefined => {
      ...
      if (command === "rev-parse --verify refs/tags/v0.6.0^{commit}") return releaseHead;
    ...
    assert.ok(calls.some((args) => args.join(" ") === "rev-parse --verify refs/tags/v0.6.0^{commit}"));
