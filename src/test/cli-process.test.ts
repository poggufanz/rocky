...
test("release truth keeps branch and immutable-tag modes distinct", async () => {
  ...
  const releaseHead = "a315bfcf71cd99af3e6cd12cfc8f14f39097e0d8";
  ...
  try {
    ...
    const runner = (_root: string, args: string[], options?: { env?: NodeJS.ProcessEnv; gitExecutable?: string }): string | undefined => {
      ...
      if (command === "rev-parse --verify refs/tags/v0.7.0^{commit}") return releaseHead;
    ...
    assert.ok(calls.some((args) => args.join(" ") === "rev-parse --verify refs/tags/v0.7.0^{commit}"));
