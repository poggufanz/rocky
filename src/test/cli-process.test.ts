...
test("release truth keeps branch and immutable-tag modes distinct", async () => {
  ...
  const releaseHead = "f20b293edcdeb92dbeac37e2fe42579e2b2c5c10";
  ...
  try {
    ...
    const runner = (_root: string, args: string[], options?: { env?: NodeJS.ProcessEnv; gitExecutable?: string }): string | undefined => {
      ...
      if (command === "rev-parse --verify refs/tags/v0.5.5^{commit}") return releaseHead;
    ...
    assert.ok(calls.some((args) => args.join(" ") === "rev-parse --verify refs/tags/v0.5.5^{commit}"));
