...
test("production identity constants match package metadata without duplicate literals", () => {
  ...
  assert.deepEqual(occurrences, [
    ...
    { path: "src/core/package-info.ts", literal: "0.6.0" },
  ]);
});
...
test("canonical release truth rejects drift in every release marker", async () => {
  ...
  const realRoadmapMarker = /^-\s+\*\*v0\.6\b[^\r\n]*\(current release\)[^\r\n]*$/im.exec(snapshot.readme)?.[0];
  ...
  const changelogHeadingRemoved = snapshot.changelog.replace(/^## 0.6.0[^\r\n]*\r?\n/im, "");
  assert.notDeepEqual(
    releaseCheck.validateReleaseTruth({
      ...snapshot,
  ...
  assert.notDeepEqual(
    releaseCheck.validateReleaseTruth({
      ...snapshot,
      packageInfoSource: `// export const PACKAGE_VERSION = "0.6.0";\n${changedRealVersion}`,
    }),
    [],
  ...
  const currentSectionStart = snapshot.changelog.indexOf("## 0.6.0");
  ...
  const mutations: Array<[string, Record<string, unknown>]> = [
    ...
    ["package-info", { ...snapshot, packageInfoSource: snapshot.packageInfoSource.replace('PACKAGE_VERSION = "0.6.0"', 'PACKAGE_VERSION = "9.9.9"') }],
    ["README", { ...snapshot, readme: snapshot.readme.replace("@poggufanz/rocky-cli@0.6.0", "@poggufanz/rocky-cli@9.9.9") }],
    ...
    ["README roadmap v0.7", { ...snapshot, readme: snapshot.readme.replace("v0.6 - his accountability", "v0.7 - his accountability") }],
    ["README roadmap v0.6.0.1", { ...snapshot, readme: snapshot.readme.replace("v0.6 - his accountability", "v0.6.0.1 - his accountability") }],
    ["README roadmap v0.6-beta", { ...snapshot, readme: snapshot.readme.replace("v0.6 - his accountability", "v0.6-beta - his accountability") }],
    ...
    ["CHANGELOG", { ...snapshot, changelog: snapshot.changelog.replace("## 0.6.0", "## 9.9.9") }],
    ...
    ["help", { ...snapshot, helpStdout: snapshot.helpStdout.replace("@poggufanz/rocky-cli@0.6.0", "@poggufanz/rocky-cli@9.9.9") }],
    ...
    ["help conflicting package", { ...snapshot, helpStdout: snapshot.helpStdout.replace("version: @poggufanz/rocky-cli@0.6.0", "version: @wrong/rocky@0.6.0") }],
