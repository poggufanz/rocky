...
test("package and lock contain no runtime or optional dependencies", () => {
  ...
  assert.equal(root.version, "0.7.3");
...
test("production identity constants match package metadata without duplicate literals", () => {
  ...
  assert.deepEqual(occurrences, [
    ...
    { path: "src/core/package-info.ts", literal: "0.7.3" },
  ]);
});
...
test("npm pack dry-run exposes only the bounded production payload", (t) => {
  ...
  assert.equal(packed.version, "0.7.3");
...
test("canonical release truth rejects drift in every release marker", async () => {
  ...
  const changelogHeadingRemoved = snapshot.changelog.replace(/^## 0\.7\.3[^\r\n]*\r?\n/im, "");
  assert.notDeepEqual(
    releaseCheck.validateReleaseTruth({
      ...snapshot,
  ...
  assert.notDeepEqual(
    releaseCheck.validateReleaseTruth({
      ...snapshot,
      packageInfoSource: `// export const PACKAGE_VERSION = "0.7.3";\n${changedRealVersion}`,
    }),
    [],
  ...
  const currentSectionStart = snapshot.changelog.indexOf("## 0.7.3");
  const nextSectionStart = snapshot.changelog.indexOf("\n## 0.7.2", currentSectionStart);
  ...
  const mutations: Array<[string, Record<string, unknown>]> = [
    ...
    ["package-info", { ...snapshot, packageInfoSource: snapshot.packageInfoSource.replace('PACKAGE_VERSION = "0.7.3"', 'PACKAGE_VERSION = "9.9.9"') }],
    ["README", { ...snapshot, readme: snapshot.readme.replace("@poggufanz/rocky-cli@0.7.3", "@poggufanz/rocky-cli@9.9.9") }],
    ...
    ["CHANGELOG", { ...snapshot, changelog: snapshot.changelog.replace("## 0.7.3", "## 9.9.9") }],
    ...
    ["help", { ...snapshot, helpStdout: snapshot.helpStdout.replace("@poggufanz/rocky-cli@0.7.3", "@poggufanz/rocky-cli@9.9.9") }],
    ...
    ["help conflicting package", { ...snapshot, helpStdout: snapshot.helpStdout.replace("version: @poggufanz/rocky-cli@0.7.3", "version: @wrong/rocky@0.7.3") }],
