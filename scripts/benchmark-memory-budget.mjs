import { closeSync, mkdirSync, mkdtempSync, openSync, rmSync, statSync, writeSync } from "node:fs";
...
function fixtureHome(root, query) {
  return join(root, query);
}

function prepareFixture(root, envelope, query) {
  const memoryHome = fixtureHome(root, query);
  const memoryPath = join(memoryHome, "memory.jsonl");
  mkdirSync(memoryHome, { recursive: true });
  writeFixture(memoryPath, envelope);
}

function measure(root, envelope, query, operation) {
  // Fixture synthesis belongs to the parent. This worker measures only the
  // existing file and bounded query, so allocator/RSS history is production-
  // shaped rather than polluted by benchmark string construction.
...
function worker(envelope, operation, root) {
  const query = `case-${String(envelope)}`;
  process.stdout.write(JSON.stringify({ envelope, ...measure(root, envelope, query, operation) }) + "\n");
}

function runWorker(envelope, operation) {
  const root = mkdtempSync(join(tmpdir(), "rocky-memory-budget-fixture-"));
  try {
    // Prepare before spawning the --expose-gc worker; the measured process
    // starts with an already materialized on-disk fixture.
    prepareFixture(root, envelope, `case-${String(envelope)}`);
    const result = spawnSync(process.execPath, ["--expose-gc", fileURLToPath(import.meta.url), "--worker", String(envelope), operation, root], {
      encoding: "utf8",
...
if (process.argv[2] === "--worker") {
  ...
  const workerRoot = process.argv[5];
  if (typeof workerRoot !== "string" || workerRoot.length === 0) {
    throw new Error("benchmark worker requires parent-prepared fixture root");
  }
  worker(workerEnvelope, process.argv[4] === "recall" ? "recall" : "stats", workerRoot);
} else {
  const scorecard = [];
  for (const envelope of [...COUNTS, "over-cap", "scalar-heavy"]) {
